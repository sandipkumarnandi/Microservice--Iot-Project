...
export class SNS_SQS {
  ...
  public async listenToService(noOfSocket,topicName, serviceName, callBack) {
    var timer =10000;
    console.log("No of Socket is: "+ noOfSocket);
    ...
    function getMessages() {
        ...
        if(noOfSocket>0) { sqs.receiveMessage(receiveMessageParams, receiveMessageCallback);
          console.log("Inside sqs.receivemessage");}
      
      
      
    }
     if(noOfSocket>0) { console.log("inside condition for setinterval");let intId =  setInterval(getMessages, timer);intervalArray.push(intId);}
     
     if(noOfSocket===0){ for( var i=0;i<intervalArray.length;i++){
    ...
    function receiveMessageCallback(err, data) {
      ...
      console.log("inside receive message callback");
